...
function decodeUplink(input) {
  try {
    d = Decode(input.fPort, input.bytes);
    d = JSON.stringify(d);
  } catch (error) {
    d = error.message;
  }
    ...
    case PROTOCOL_VERSION_V2:
      ...
      switch (decoded.header.message_type) {
        ...
        case STR_SENSOR_DATA:
          decoded.sensor_data = decode_sensor_data_msg(
            bytes,
            cursor,
    ...
    case PROTOCOL_VERSION_V3:
      ...
      switch (fPort) {
        ...
        case FPORT_CONFIG_UPDATE:
          decoded.config_update_ans = decode_config_update_ans_msg(
            bytes,
            cursor
          );
          break;
...
function decode_sensor_data_config(bytes, cursor, protocol_version) {
  ...
  if (result.spectral_line_frequency == 0) {
    throw new Error(
      "Invalid spectral_line_frequency value in sensor data config!"
    );
  }
...
function data_scale_lookup(scale_idx) {
  var scale_value = [
    0.01, 0.0108175019990394, 0.0117018349499221, 0.0126584622963211,
    0.0136932941195217, 0.014812723651136, 0.0160236667707382,
    0.0173336047324401, 0.0187506303843729, 0.0202834981666202,
    0.0219416781964925, 0.0237354147752836, 0.0256757896779659,
    0.027774790616831, 0.0300453853020469, 0.0325016015566801,
    0.0351586139811367, 0.03803283770244, 0.0411420297875284,
    0.0445053989471126, 0.0481437242078435, 0.0520794832859547,
    0.0563369914554751, 0.0609425517689466, 0.0659246175587139,
    0.0713139682227294, 0.0771438993808804, 0.0834504285766365,
    0.0902725177948457, 0.097652314170406, 0.105635410374919, 0.114271126290003,
    0.123612813707458, 0.133718185938731, 0.144649674370014, 0.156474814165802,
    0.169266661503788, 0.183104244918794, 0.198073053544165, 0.214265565266983,
    0.231781818060089, 0.250730028020599, 0.271227257933203, 0.293400140488639,
    0.317385660625428, 0.3433320018282, 0.371399461611073, 0.401761441841993,
    0.434605520026269, 0.470134608167771, 0.508568206367245, 0.550143758902554,
    0.59511812116874, 0.64376914654074, 0.696397402962432, 0.753328029867193,
    0.814912746902074, 0.881532026865585, 0.953597446283568, 1.03155422814513,
    1.11588399250775, 1.20710773196486, 1.30578903035857, 1.41253754462275,
    1.52801277126748, 1.65292812077436, 1.78805532507451, 1.93422920533864,
    2.09235282953511, 2.26340309161917, 2.44843674682223, 2.64859694032709,
    2.86512026966378, 3.09934442445761, 3.35271645072817, 3.62680169079642,
    3.92329345403096, 4.24402347817979, 4.59097324591799, 4.96628622652541,
    5.37228111832403, 5.81146617368716, 6.28655469512105, 6.80048179815422,
    7.35642254459641, 7.95781155819499, 8.60836424387529, 9.31209974165798,
    10.0733657570639, 10.8968654214094, 11.7876863479359, 12.7513320632845,
    13.7937560084995, 14.9213983196205, 16.1412256150957, 17.4607740358243,
    18.8881958037304, 20.4323095865101, 22.1026549797064, 23.9095514427051,
    25.8641620527597, 27.9785624709206, 30.2658155459431, 32.7400520170796,
    35.4165578143412, 38.3118684955729, 41.4438714037792, 44.8319161758313,
    48.496934285282, 52.4615683578319, 56.7503120583586, 61.3896614137401,
    66.4082785063484, 71.8371685495186, 77.7098714389746, 84.0626689636199,
    90.9348089558542, 98.3687477662216, 106.41041256041, 115.109485059084,
    124.519708473503, 134.699219533192, 145.710907656935, 157.622803486073,
...
function decode_boot_msg(bytes, cursor) {
  ...
  // byte[37..44]
  boot.sensor.reboot_info = decode_reboot_info(
    sensor_reboot_type,
    bytes,
    cursor
  );
...
function decode_boot_msg_v3(bytes, cursor) {
  ...
  if (
    bytes.length != expected_length_normal &&
    bytes.length != expected_length_debug
  ) {
    throw new Error(
      "Invalid boot message length " +
        bytes.length +
...
function decode_activated_msg(bytes, cursor) {
  ...
  if (bytes.length != expected_length) {
    throw new Error(
      "Invalid activated message length " +
        bytes.length +
        " instead of " +
        expected_length
    );
...
function decode_deactivated_msg(bytes, cursor) {
  ...
  if (bytes.length != expected_length) {
    throw new Error(
      "Invalid deactivated message length " +
        bytes.length +
        " instead of " +
        expected_length
    );
...
function decode_sensor_event_msg(bytes, cursor) {
  ...
  if (bytes.length != expected_length) {
    throw new Error(
      "Invalid sensor_event message length " +
        bytes.length +
        " instead of " +
        expected_length
    );
...
function decode_device_status_msg(bytes, cursor) {
  ...
  device_status.base.battery_voltage.low =
    decode_uint16(bytes, cursor) / 1000.0;
  device_status.base.battery_voltage.high =
    decode_uint16(bytes, cursor) / 1000.0;
  device_status.base.battery_voltage.settle =
    decode_uint16(bytes, cursor) / 1000.0;
...
function decode_device_status_msg_v3(bytes, cursor) {
  ...
  if (
    ...
    throw new Error(
      "Invalid device status message length " +
        bytes.length +
        " instead of " +
        expected_length_normal +
        " or " +
...
function decode_config_update_ans_msg(bytes, cursor) {
  ...
  if (bytes.length != expected_length) {
    throw new Error(
      "Invalid config update ans message length " +
        bytes.length +
        " instead of " +
        expected_length
    );
...
function decode_sensor_data_msg(bytes, cursor, protocol_version) {
  ...
  if (bytes.length != expected_length) {
    throw new Error(
      "Invalid sensor_data message length " +
        bytes.length +
        " instead of " +
        expected_length
    );
  ...
  if (protocol_version == 3) {
    ...
    // byte[1..5]
    sensor_data.config = decode_sensor_data_config(
      bytes,
      cursor,
      protocol_version
    );
  ...
  for (i = 0; i < chunk_size; i++) {
    ...
    sensor_data.magnitude[i] =
      (sensor_data.raw[i] * sensor_data.config.scale) / 255;
  }
    ...
    case FPORT_FIRMWARE_MANAGEMENT_PROTOCOL_SPECIFICATION: {
      ...
      switch (cid) {
        case CID_DEV_VERSION:
          decoded.DevVersion = {
            FW_version: "0x" + uint32_to_hex(decode_uint32(bytes, cursor)),
            HW_version: "0x" + uint32_to_hex(decode_uint32(bytes, cursor)),
          };
          break;
      }
      return true;
